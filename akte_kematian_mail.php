...
if (isset($_POST["submit"])){
    ...
    $path4 = 'upload/' . $_FILES["suratkematian"]["name"];
    move_uploaded_file($_FILES["suratkematian"]["tmp_name"], $path4);
    $path5 = 'upload/' . $_FILES["aktelahir"]["name"];
    move_uploaded_file($_FILES["aktelahir"]["tmp_name"], $path5);
    $path6 = 'upload/' . $_FILES["suratnikah"]["name"];
    move_uploaded_file($_FILES["suratnikah"]["tmp_name"], $path6);
    $path7 = 'upload/' . $_FILES["pemakaman"]["name"];
    move_uploaded_file($_FILES["pemakaman"]["tmp_name"], $path7);
    $path8 = 'upload/' . $_FILES["ktppelapor"]["name"];
    move_uploaded_file($_FILES["ktppelapor"]["tmp_name"], $path8);
    $message = '
    <h3 align="center">Permohonan Akte Kematian</h3>
...
// ADD YOUR DETAILS HERE
    ...
    $mail->AddAttachment($path7); 
    $mail->AddAttachment($path8); //Adds an attachment from a path on the filesystem
    $mail->Subject = 'Permohonan Akte Kematian';                //Sets the Subject of the message
